...
object LayerHistogram extends LazyLogging {
  ...
  def apply[Param](
    ...
    for {
      ...
      _                 <- IO { logger.error(s"[LayerHistogram] Intersection of provided layer extents calculated: $intersection") }
      cellSize          <- IO { SampleUtils.chooseLargestCellSize(rasterExtents.map(_.cellSize)) }
      _                 <- IO { logger.error(s"[LayerHistogram] Largest cell size of provided layers calculated: $cellSize") }
      mbtileForExtent   <- IO { LayerExtent(getExpression, getParams, interpreter) }
      _                 <- IO { logger.error(s"[LayerHistogram] calculating histogram from (approximately) ${intersection.area / (cellSize.width * cellSize.height)} cells") }
